...
const ZWGPUWorkaroundCopyTextureAndMap = extern struct {
    device: c.WGPUDevice,
    queue: c.WGPUQueue,
    instance: c.WGPUInstance,
    render_texture: c.WGPUTexture,
...
pub export fn _zwgpuCopyTextureAndMap(workaround: *ZWGPUWorkaroundCopyTextureAndMap) void {
    const cmd_encoder_desc = c.WGPUCommandEncoderDescriptor{
        .nextInChain = null,
        .label = makeStringView("Copy Command Encoder"),
    };
    const copy_cmd_encoder = zwgpuDeviceCreateCommandEncoder(workaround.device, &cmd_encoder_desc);
    if (copy_cmd_encoder == null) {
        std.debug.print("Failed to create copy command encoder\n", .{});
        return;
    }
    defer zwgpuCommandEncoderRelease(copy_cmd_encoder);

    const copy_src = c.WGPUTexelCopyTextureInfo{
        .texture = workaround.render_texture,
        .mipLevel = 0,
    ...
    const copy_dst = c.WGPUTexelCopyBufferInfo{
        .layout = c.WGPUTexelCopyBufferLayout{
            .offset = 0,
            .bytesPerRow = workaround.bytes_per_row,
            .rowsPerImage = workaround.height,
        },
        .buffer = workaround.readback_buffer,
    };
    const copy_size = c.WGPUExtent3D{ .width = workaround.width, .height = workaround.height, .depthOrArrayLayers = 1 };
    ...
    if (mapped_range) |range| {
        const mapped_data = @as([*]const u8, @ptrCast(range));
        @memcpy(workaround.output_buffer[0..workaround.buffer_size], mapped_data[0..workaround.buffer_size]);
    }
